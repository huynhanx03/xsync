...
func (m *Map[V]) Delete(key uint64) bool {
	_, ok := m.DeleteAndGet(key)
	return ok
}

// DeleteAndGet removes a key and returns the previous value if present.
func (m *Map[V]) DeleteAndGet(key uint64) (uint64, bool) {
...
retry:
	...
	if !found {
		targetSlotIdx, targetSlot, targetVal = findKeyInLinks(idx, pb, h0, key)
...
func findKeyInLinkBucket[V Integer](b *LinkBucket[V], vm4 uint32, key uint64, baseSlotIdx int) (int, *Slot[V], uint64) {
	k0 := atomicLoadSlotKey(&b.Slots[0])
	k1 := atomicLoadSlotKey(&b.Slots[1])
	k2 := atomicLoadSlotKey(&b.Slots[2])
	k3 := atomicLoadSlotKey(&b.Slots[3])
