...
func atomicLoadSlotVal[V Integer](slot *Slot[V]) uint64 {
	return atomic.LoadUint64(&slot.Val)
}

func atomicStoreSlotVal[V Integer](slot *Slot[V], val uint64) {
	atomic.StoreUint64(&slot.Val, val)
}
