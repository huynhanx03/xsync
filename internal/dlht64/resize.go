...
//
// TODO(dlht): once prefetch support exists, prefetch destination bins while
// transferring link buckets. Resize is not the main hot path, but large
// memory-resident tables can pay noticeable migration latency.
func (m *Map[V]) transferValidSlots(ctx *resizeContext[V], vm uint32, baseSlot *Slot[V], sentinel uint64) {
	for vm != 0 {
		...
		actualKey := atomicLoadSlotKey(slot)
		...
		hash := hashUint64(m.hashConfig.Seed, actualKey)
